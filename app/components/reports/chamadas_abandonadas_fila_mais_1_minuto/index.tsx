...
export default function ChamadasAbandonadasEmFilaMais1Minuto({ startDate, endDate, sortOrder, onLoadingChange }: ChamadasCurtasProps) {
  ...
  const fetchData = async () => {
    ...
    console.log('🚀 [ChamadasAbandonadasNaFilaMais1Min] Iniciando busca de dados...');
    ...
    try {
      ...
      if (result.success) {
        console.log('✅ [ChamadasAbandonadasNaFilaMais1Min] Dados recebidos:', result.data?.length, 'registros');
        setData(result.data || []);
        setCurrentPage(1);
      } else {
        console.error('❌ [ChamadasAbandonadasNaFilaMais1Min] Erro na resposta:', result.error);
        alert('Erro ao carregar dados: ' + result.error);
      }
    } catch (error) {
      console.error('💥 [ChamadasAbandonadasNaFilaMais1Min] Erro na requisição:', error);
    ...
    return (
      <div className="bg-white rounded-lg shadow p-6 text-center">
        <p className="text-gray-500 text-sm">
          Nenhuma chamada abandonada em fila superior a 1 minuto encontrada para o período selecionado
        </p>
      </div>
    );
  }
